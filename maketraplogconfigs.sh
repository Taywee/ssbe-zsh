...
# the agents
# SSBE_USER, SSBE_PASS, AGENT_PASS, and CLIENT are expected to be set as environment variables  (or set in .ssbe), otherwise this probably won't work out for you
...
echo "cd /opt/SysShep" >&3
echo "tar -xvzf SysShep-s-UBUNTU-5.3.7.tar.gz" >&3
echo "export LD_LIBRARY_PATH=../lib" >&3
echo >&3
...
cd ${HOST}-trap/bin
./configure "${SSBE_USER}" "${SSBE_PASS}"  ssbe06.qwest.sysshep.com ${CLIENT} ${HOST}-trap '${CONFIG}' '${AGENT_PASS}' && ./sysshepctl start
